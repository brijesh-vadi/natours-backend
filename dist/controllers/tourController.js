...
export const validateTourBody = async (req, res, next) => {
    try {
        ...
        const tour = tours.rows.find((tour) => tour.name === req.body.name);
        if (tour) {
            res.status(409).json({
                status: 'fail',
...
export const createTour = async (req, res) => {
    const { name, price, description, summary, difficulty, duration, imageCover, images, maxGroupSize, startDates, priceDiscount, ratingsAverage, ratingsQuantity, } = req.body;
    try {
        const query = `
...
export const updateTour = async (req, res) => {
    ...
    const query = 'UPDATE tours SET name = $1,price = $2,rating = $3,updated_at = CURRENT_TIMESTAMP WHERE id = $4 RETURNING *';
    try {
        const tour = await pool.query('SELECT * FROM tours WHERE id = $1', [tourId]);
